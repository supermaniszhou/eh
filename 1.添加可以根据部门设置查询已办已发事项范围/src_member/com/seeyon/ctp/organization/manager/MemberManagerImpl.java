...
import com.seeyon.ctp.common.authenticate.UserPasswordManager;
...
/*        if (params.containsKey("showByType") && null != params.get("showByType")
        ...
        if (params.containsKey("deptId") && Strings.isNotBlank(String.valueOf(params.get("deptId"))) && !Long.valueOf(params.get("deptId").toString()).equals(accountId)) {
            if (params.containsKey("subMembers") && params.get("subMembers") != null && Boolean.valueOf(params.get("subMembers").toString())) {
                queryParams.put("orgDepartmentId_includeChildren", Long.parseLong(String.valueOf(params.get("deptId"))));
            } else {
                queryParams.put("orgDepartmentId", Long.parseLong(String.valueOf(params.get("deptId"))));
            }
        }
    ...
    public HashMap viewOne(Long memberId) throws BusinessException {
        ...
        map.put("name", member.getI18nNameId() == null ? member.getName() : member.getI18nNameId());
        map.put("name_text", member.getName());//多语言插件渲染出来的text
        ...
        if (null != map.get("reporter")
            ...
            if (null != reporter) {
                ...
                //回写汇报人所在部门title
                map.put("reporterDeptTitle", orgManager.showDepartmentFullPath(reporter.getOrgDepartmentId()));
            } else {
            ...
            if (OrgConstants.ROLE_BOND.DEPARTMENT.ordinal() == m.getRole().getBond()
                ...
                if (isShowRoleByCategory) {
                    if (!"0".equals(m.getRole().getCategory().trim())) {
                        ...
                        if (!member.getOrgAccountId().equals(m.getAccountId()) || roleIds.indexOf(m.getRole().getId().toString()) != -1)
                            continue;
                        roles.append(m.getRole().getShowName()).append(",");
                        roleIds.append(m.getRole().getId()).append(",");
                    }
            ...
            try {
                ...
                if (null != addressBook) {
                    ...
                    if (metadataColumn.getType() == 3) {//枚举
                        ...
                        map.put(key, saveValue);
/*							 String txt = "";
							 if(Strings.isNotBlank(saveValue)){
								 try {
									 txt = enumManagerNew.getEItemNameById(Long.valueOf(saveValue));
								} catch (Exception e) {
								}
							 }
							 map.put(key+"_txt",txt);*/
                    }
                    if (metadataColumn.getType() == 4) {//选人
                        ...
                        if (Strings.isNotBlank(saveValue)) {
                            try {
                                txt = orgManager.getMemberById(Long.valueOf(saveValue)).getName();
                            } catch (Exception e) {
                            }
                        }
                        map.put(key + "_txt", txt);
                    ...
                    if (metadataColumn.getType() == 5) {//选部门
                        ...
                        if (Strings.isNotBlank(saveValue)) {
                            try {
                                txt = orgManager.getDepartmentById(Long.valueOf(saveValue)).getName();
                            } catch (Exception e) {
                            }
                        }
                        map.put(key + "_txt", txt);
    ...
    public Object createMember(String accountId, Map map) throws BusinessException {
        ...
        if (SystemEnvironment.functionSwitch(Plugins.DATA_I18N)) {
            if (null != map.get("name") && Strings.isNotBlank((String) map.get("name"))) {
                String i18nId = (String) map.get("name");
                if (null != map.get("i18nEnable") && Boolean.valueOf(map.get("i18nEnable").toString()) && Strings.isLong(i18nId)) {
                    member.setI18nNameId(Long.valueOf(i18nId));
                }
            }
        }
        if (null != map.get("name_text") && Strings.isNotBlank((String) map.get("name_text"))) {
            member.setName((String) map.get("name_text"));
        }
    ...
    private void checkRole4DeptSpace(Map map) throws BusinessException {
        ...
        if (isDeptManager && !rIds.contains(deptAdmin.getId())
            ...
            if (dept != null && dept.CreateDeptSpace()) {
     ...
     * @throws BusinessException
     */
    private void checkRole4DeptSpaceToOuter(V3xOrgMember member) throws BusinessException {
        Long id = member.getId();
        Long orgAccountId = member.getOrgAccountId();

        Map<Long, List<MemberRole>> depManagerMemberRoleMap = new HashMap<Long, List<MemberRole>>();
        List<MemberRole> memberRoles = orgManager.getMemberRoles(id, null);
        for (MemberRole memberRole : memberRoles) {
            if (OrgConstants.Role_NAME.DepManager.name().equals(memberRole.getRole().getCode())) {
                Long accountId = memberRole.getAccountId();
                List<MemberRole> list = depManagerMemberRoleMap.get(accountId);
                if (list == null) {
                    list = new ArrayList<MemberRole>();
                    depManagerMemberRoleMap.put(accountId, list);
                }
                list.add(memberRole);
            }
        }

        if (!depManagerMemberRoleMap.isEmpty()) {
            V3xOrgRole depManager = null;
            //先取主岗单位的部门主管进行判断
            List<MemberRole> memberRoleList = depManagerMemberRoleMap.get(orgAccountId);
            if (Strings.isNotEmpty(memberRoleList)) {
                depManager = orgManager.getRoleByName(OrgConstants.Role_NAME.DepManager.name(), orgAccountId);
                if (depManager != null && depManager.isValid()) {
                    for (MemberRole mr : memberRoleList) {
                        Long deptId = mr.getDepartment().getId();
                        V3xOrgDepartment dept = orgManager.getDepartmentById(deptId);

                        if (dept != null && dept.CreateDeptSpace()) {
                            List<V3xOrgMember> members = orgManager.getMembersByRole(dept.getId(), depManager.getId());
                            if (Strings.isNotEmpty(members) && members.size() == 1 && members.get(0).getId().equals(id)) {//如果只有当前这一个人是部门主管，则不能转编外人员
                                throw new BusinessException(ResourceUtil.getString("dept.space.mustmanager.member.toOuter", dept.getName(), depManager.getShowName()));
                            }
                        }
                    }
                }
            ...
            //当前单位的部门主管校验通过后，再校验兼职单位的部门主管数据
            for (Long accountId : depManagerMemberRoleMap.keySet()) {
                if (accountId.equals(orgAccountId)) {
                    continue;
                }
                V3xOrgAccount account = orgManager.getAccountById(accountId);
                memberRoleList = depManagerMemberRoleMap.get(accountId);
                if (Strings.isNotEmpty(memberRoleList)) {
                    depManager = orgManager.getRoleByName(OrgConstants.Role_NAME.DepManager.name(), orgAccountId);
                    if (depManager != null && depManager.isValid()) {
                        for (MemberRole mr : memberRoleList) {
                            ...
                            if (dept != null && dept.CreateDeptSpace()) {
                                List<V3xOrgMember> members = orgManager.getMembersByRole(dept.getId(), depManager.getId());
                                if (Strings.isNotEmpty(members) && members.size() == 1 && members.get(0).getId().equals(id)) {//如果只有当前这一个人是部门主管，则不能转编外人员
                                    throw new BusinessException(ResourceUtil.getString("dept.space.mustmanager.member.toOuter", dept.getName() + "(" + account.getShortName() + ")", depManager.getShowName()));
                                }
                            }
                        }
                    }
    ...
    public Object updateMember(Map map) throws BusinessException {
        ...
        updateMember.setI18nNameId(oldMember.getI18nNameId());

        if (SystemEnvironment.functionSwitch(Plugins.DATA_I18N)) {
            if (updateMember.getI18nNameId() == null) {
                String i18nId = (String) map.get("name");
                if (null != map.get("i18nEnable") && Boolean.valueOf(map.get("i18nEnable").toString()) && Strings.isLong(i18nId)) {
                    updateMember.setI18nNameId(Long.valueOf((String) map.get("name")));
                }
            }
        }
        if (null != map.get("name_text") && Strings.isNotBlank((String) map.get("name_text"))) {
            updateMember.setName((String) map.get("name_text"));
        }

        boolean innerToOuter = false;
        ...
        boolean passwordchange = (null != map.get("isChangePWD") && Boolean.valueOf(map.get("isChangePWD").toString()))
                || null == oldPrincipal || !map.get("loginName").toString().equals(oldPrincipal.getLoginName())
                || !OrgConstants.DEFAULT_INTERNAL_PASSWORD.equals(password);
        if (passwordchange) {
            //loginName
            V3xOrgPrincipal p = new V3xOrgPrincipal(updateMember.getId(), map.get("loginName").toString(), password);
            updateMember.setV3xOrgPrincipal(p);
            UserPasswordManager upm = UserPasswordManager.getInstance();
            upm.setLoginPasswordStrength(updateMember.getId(), upm.checkStrength(password));
        }
...
/*        if(!updateMember.getEnabled()
        ...
        for (MemberRole m : memberRoles) {
            ...
            if ("0".equals(role.getCategory().trim())) {
                if (OrgConstants.Role_NAME.DepManager.name().equals(role.getCode())) {
        ...
        if (updateMember.getIsInternal()) {//内部人员读取roleIds
            ...
            if (hasOuterRole) {
                ...
                String roleIds = (String) map.get("extRoleIds");
                if (innerToOuter) {//内部人员转编外人员,部门主管角色取消判断
                    checkRole4DeptSpaceToOuter(oldMember);
                }
        ...
        if (updateMember.getIsInternal()) {//内部人员读取roleIds
            ...
            String roleIds = (String) map.get("extRoleIds");
            if (innerToOuter) {//内部人员转编外人员,清除所有的角色信息，不做校验。
                List<V3xOrgRelationship> rels = orgCache.getV3xOrgRelationship(OrgConstants.RelationshipType.Member_Role, oldMember.getId(), null, null);
        ...
        if (innerToOuter) {//内部人员转编外人员
            ...
            objectiveIds.put(OrgConstants.RelationshipObjectiveName.objective0Id, updateMember.getId());
            objectiveIds.put(OrgConstants.RelationshipObjectiveName.objective5Id, OrgConstants.ORGENT_TYPE.Member.name());
            orgDao.deleteOrgRelationshipPO(OrgConstants.RelationshipType.BusinessDepartment_Member.name(), null, null, objectiveIds);
            //2.删除系统组中的该人员
            objectiveIds = new EnumMap<RelationshipObjectiveName, Object>(RelationshipObjectiveName.class);
            objectiveIds.put(OrgConstants.RelationshipObjectiveName.objective0Id, updateMember.getId());
            objectiveIds.put(OrgConstants.RelationshipObjectiveName.objective6Id, OrgConstants.ORGENT_TYPE.Member.name());
            orgDao.deleteOrgRelationshipPO(OrgConstants.RelationshipType.Team_Member.name(), null, null, objectiveIds);

            //3.删除这个人的所有兼职副岗信息。
            objectiveIds = new EnumMap<RelationshipObjectiveName, Object>(RelationshipObjectiveName.class);
            objectiveIds.put(OrgConstants.RelationshipObjectiveName.objective5Id, OrgConstants.MemberPostType.Second.name());
            orgDao.deleteOrgRelationshipPO(OrgConstants.RelationshipType.Member_Post.name(), updateMember.getId(), null, objectiveIds);

            objectiveIds = new EnumMap<RelationshipObjectiveName, Object>(RelationshipObjectiveName.class);
            objectiveIds.put(OrgConstants.RelationshipObjectiveName.objective5Id, OrgConstants.MemberPostType.Concurrent.name());
            orgDao.deleteOrgRelationshipPO(OrgConstants.RelationshipType.Member_Post.name(), updateMember.getId(), null, objectiveIds);

            appLogManager.insertLog4Account(user, user.isGroupAdmin() ? currentAccountId : AppContext.currentAccountId(), -807, user.getName(), updateMember.getName());

            //添加监听，对 工作管理权限,业务管理权限,使用授权类的相关数据进行清理。(管理员通过工作交接处理，不再后台清理相关授权)
//    		MemberInnerToOuterEvent memberInnerToOuterEvent= new MemberInnerToOuterEvent(this);
//    		memberInnerToOuterEvent.setMember(updateMember);
//        	EventDispatcher.fireEvent(memberInnerToOuterEvent);
        }
        ...
        privilegeMenuManager.updateInnerMemberMenuLastDate(updateMember.getId(), updateMember.getOrgAccountId());
        return returnMessage.getSuccessMsgs().get(0).getEnt().getId();
    }
    ...
    private void dealRoles(Long currentAccountId, V3xOrgMember updateMember, String roleIds) throws BusinessException {
        ...
        if (!validRole.isEmpty() && !validRole.contains(defultRole.getId())) {
            ...
            if (OrgConstants.ROLE_BOND.DEPARTMENT.ordinal() == defultRole.getBond()) {
                objectiveIds.put(OrgConstants.RelationshipObjectiveName.objective0Id, updateMember.getOrgDepartmentId());
            } else {
    ...
    public Object cancelMember(Long[] ids) throws BusinessException {
        ...
        if (LdapUtils.isLdapEnabled()) {
            try {
                ...
                logger.error("LDAP_AD delete bind data error", e);
            }
        }
        ...
        appLogManager.insertLogs4Account(user, user.isGroupAdmin() ? members.get(0).getOrgAccountId() : AppContext.currentAccountId(), AppLogAction.Organization_MoveMember, appLogs);
        return true;
    }
    ...
    public FlipInfo showExtMember(FlipInfo fi, Map params) throws BusinessException {
        ...
        Long accountId = Long.parseLong(params.get("accountId").toString());
        List<OrgMember> members = orgDao.getAllMemberPOByAccountId(accountId, null, false, enabled, condition, value, null);
        ...
        for (V3xOrgMember bo : extMembers) {
            ...
            if (SystemEnvironment.functionSwitch(Plugins.DATA_I18N)) {
                o.setName(bo.getFullName().replace(OrgConstants.DATAI18N_SPLIT_FLAG, "/"));
            } else {
...
//                	if (!organizationLdapEvent.getLdapAdExUnitCode(memberLdap.getLoginName()).equals(ldapUserCodes)) 
                {
                    ...
                    //审计日志
                    appLogManager.insertLog4Account(user, user.isGroupAdmin() ? oldMember.getOrgAccountId() : AppContext.currentAccountId(), AppLogAction.LDAP_Account_Bing_Create, memberLdap.getName(),
                            ldapUserCodes);
                ...
                logger.error("ldap/ad add member bind data error", e);
            }
        } else {
            return;
        }
    ...
    public boolean checkNoRoles(String entityIds) throws BusinessException {
        ...
        for (V3xOrgRelationship r : rels) {
            ...
            if (null != tempRole
                    ...
                    && OrgConstants.ROLE_BOND.REPORTSPACE.ordinal() != tempRole.getBond()) {
                return false;
            }
        }
        return true;
    ...
    public List<ResultObject> uploadMemberPicAttachment(String zipFileName, V3XFile v3xFile, Long accountId,
        ...
        try {// 解压文件
            ...
            logger.info("------------system default charset：" + java.nio.charset.Charset.defaultCharset().toString());
